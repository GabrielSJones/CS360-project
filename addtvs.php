...
<?php
	...
	echo "<center>";
	echo "<table>";
	echo "<tr> <th>ID</th> <th>Name</th> <th>Size</th> <th>Screen Type</th> <th>HDR</th> <th>Resolution</th> <th>Refresh Rate</th> <th>Manufacturer</th> </tr>";
	echo "<tr> <td>$id</td> <td>$name</td> <td>$size</td> <td>$screenType</td> <td>$hdr</td> <td>$resolution</td> <td>$refreshRate</td> <td>$manufacturer</td> </tr>";
	echo "</table>";
	echo "</center>";
	...
	$idCheck = "SELECT * FROM Televisions WHERE TVID = '$id'";
	$checkResults = $conn->query($idCheck);
	// if it's not, then add the row to the table
	if ($checkResults->num_rows < 1)
	{
		$query = "INSERT INTO Televisions (TVID, Name, Size, ScreenType, HDR, Resolution, RefreshRate, Manufacturer) VALUES ('$id', '$name', '$size', '$screenType', '$hdr', '$resolution', '$refreshRate', '$manufacturer')";
